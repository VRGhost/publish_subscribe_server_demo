...
class SubscriberResource(Resource):
    ...
    def render_GET(self, request):
        ...
        if len(request.postpath) != 2:
            return self._interfaceError(
                request, "You must provide two-element path: /<topic>/<username>")
        ...
        try:
            ...
            # Smart client might not get below string as 204 (No Content) code implies
            #   that there should be no content returned to the client.
    ...
    def render_POST(self, request):
        ...
        if len(request.postpath) not in (1, 2):
            return self._interfaceError(
                request, "You must provide (1): two-element path '/<topic>/<username>'" \
                " to subscribe (2): '/<topic>' to post"
            )
    ...
    def _interfaceError(self, request, message):
        """Handler that is executed when the request does not correspond to any
             of the interfaces declared in the Requirements.
        """
        # Returning HTTP error code 400 (Bad request) would be appropriate in this case.
        # But I've always wanted to see what the browser will do
        # if the code 418 ("I'm a teapot"/RFC 2324) is returned
